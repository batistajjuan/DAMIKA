...
,(vd.DTV_CANTIDAD*vd.DTV_PRECIO_ORIGINAL)-(vd.DTV_CANTIDAD*vd.DTV_PRECIO_ORIGINAL*((DTV_PORC_DSCTO/100)))+(((vd.DTV_CANTIDAD*vd.DTV_PRECIO_ORIGINAL)-((vd.DTV_CANTIDAD*vd.DTV_PRECIO_ORIGINAL)*(DTV_PORC_DSCTO/100)))*(VD.DTV_IVA/100)) as IMPORTE
...
inner join CON_TIPO_DOCUMENTO CD ON CD.TDC_CODIGO = V.VTA_COD_TIPO_DOCUMENTO and CD.TDC_FISCAL IN (0, 1) and CD.TDC_OFICIAL = 1
where VTA_ANULADO=0 AND CD.TDC_CODIGO IN ('01','02','04','E1','E2','E3','E4','PR')
...
def build_products(prod_sales, prod_margin, prod_qty, max_mo):
    ...
    for yr in [2024, 2025, 2026]:
        ...
        with_sales   = [r for r in rows if r['s'] > 0]
        by_margin_hi = sorted(with_sales, key=lambda x: -x['p'])[:10]
        by_margin_lo = sorted(with_sales, key=lambda x:  x['p'])[:10]
